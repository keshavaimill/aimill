import React from "react";
import { Link } from "react-router-dom";
import { Navbar } from "@/components/Navbar";
import { Footer } from "@/components/Footer";
import { 
  ArrowRight,
  TrendingUp,
  Database,
  Brain,
  BarChart3,
  Package,
  LineChart,
  TrendingDown,
  Clock,
  AlertTriangle,
  Zap,
  CheckCircle,
  Activity,
  Target,
  Award,
} from "lucide-react";

const ForecastingEngine = () => {
  const problems = [
    {
      icon: AlertTriangle,
              title: "Inaccurate demand forecasts",
      desc: "Traditional models miss trends and seasonality patterns",
      color: "text-indigo-400",
      bg: "bg-indigo-500/10",
            },
            {
      icon: Package,
              title: "Excess inventory",
      desc: "Overstocking ties up capital and warehouse space",
      color: "text-indigo-400",
      bg: "bg-indigo-500/10",
            },
            {
      icon: TrendingDown,
              title: "Stockouts and lost sales",
      desc: "Understocking leads to missed opportunities and customer churn",
      color: "text-indigo-400",
      bg: "bg-indigo-500/10",
            },
            {
      icon: Clock,
              title: "Manual planning processes",
      desc: "Time-consuming and error-prone analysis delays decisions",
      color: "text-indigo-400",
      bg: "bg-indigo-500/10",
    },
  ];

  const channels = ["ERP", "WMS", "Analytics Platforms", "APIs", "Point of Sale", "E-commerce"];

  return (
    <div className="min-h-screen bg-[#020617] text-slate-300">
      <Navbar />
      <main>
        <style>{`
          @keyframes fadeInUp {
            from { opacity: 0; transform: translateY(20px); }
            to { opacity: 1; transform: translateY(0); }
          }
          @keyframes fadeInLeft {
            from { opacity: 0; transform: translateX(-40px); }
            to { opacity: 1; transform: translateX(0); }
          }
          @keyframes scaleIn {
            from { opacity: 0; transform: scale(0.95); }
            to { opacity: 1; transform: scale(1); }
          }
          @keyframes pulse {
            0%, 100% { opacity: 1; }
            50% { opacity: 0.5; }
          }
          .animate-fadeInLeft { animation: fadeInLeft 0.8s ease-out forwards; }
          .animate-scaleIn { animation: scaleIn 0.8s ease-out forwards; }
          .animate-pulse { animation: pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite; }
          .delay-200 { animation-delay: 0.2s; }
          .opacity-0 { opacity: 0; }
        `}</style>

        {/* Hero Section */}
        <section className="relative min-h-screen flex items-center overflow-hidden bg-gradient-to-b from-slate-900 via-slate-950 to-[#020617] pt-20">
          <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-500/30 rounded-full blur-3xl pointer-events-none animate-pulse" />
          <div className="absolute bottom-0 left-0 w-96 h-96 bg-violet-700/30 rounded-full blur-3xl pointer-events-none animate-pulse" style={{ animationDelay: '1s' }} />
          <div className="absolute top-1/2 left-1/2 w-[600px] h-[600px] bg-indigo-400/10 rounded-full blur-[120px] pointer-events-none -translate-x-1/2 -translate-y-1/2" />
          
          <div className="container mx-auto max-w-7xl px-6 py-20">
            <div className="grid lg:grid-cols-2 gap-16 items-center">
              <div className="animate-fadeInLeft">
                <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-500/20 border border-indigo-500/40 text-indigo-300 text-xs font-bold tracking-widest mb-6 uppercase shadow-lg shadow-indigo-500/20">
                  SUPPLY CHAIN & RETAIL
                </div>
                <h1 className="text-5xl lg:text-[64px] font-extrabold leading-[1.1] text-white mb-8 tracking-tight">
                  Demand Prediction<br />You Can Act On
                </h1>
                <p className="text-xl lg:text-2xl text-slate-300 leading-relaxed mb-6 max-w-[600px]">
                  Agentic forecasting that blends statistical models and AI reasoning to improve planning accuracy — reducing inventory costs while preventing stockouts.
                </p>
                <div className="flex flex-wrap gap-4 mb-12">
                  <div className="flex items-center gap-2 px-4 py-2 bg-indigo-500/20 rounded-full border border-indigo-500/40 shadow-md shadow-indigo-500/10 hover:bg-indigo-500/30 transition-all">
                    <TrendingUp className="w-4 h-4 text-indigo-300" />
                    <span className="text-sm font-semibold text-indigo-300">40% better accuracy</span>
                  </div>
                  <div className="flex items-center gap-2 px-4 py-2 bg-indigo-500/20 rounded-full border border-indigo-500/40 shadow-md shadow-indigo-500/10 hover:bg-indigo-500/30 transition-all">
                    <Package className="w-4 h-4 text-indigo-300" />
                    <span className="text-sm font-semibold text-indigo-300">30% lower inventory</span>
                  </div>
                  <div className="flex items-center gap-2 px-4 py-2 bg-indigo-500/20 rounded-full border border-indigo-500/40 shadow-md shadow-indigo-500/10 hover:bg-indigo-500/30 transition-all">
                    <Target className="w-4 h-4 text-indigo-300" />
                    <span className="text-sm font-semibold text-indigo-300">50% fewer stockouts</span>
                  </div>
                </div>
                <Link 
                  to="/contact"
                  className="inline-flex items-center gap-2 h-14 px-8 text-lg font-semibold bg-gradient-to-br from-indigo-500 via-indigo-600 to-violet-700 text-white rounded-xl shadow-xl shadow-indigo-500/40 hover:scale-105 hover:shadow-2xl hover:shadow-indigo-500/50 transition-all"
                >
                  Talk to Supply Chain AI Experts <ArrowRight className="w-5 h-5" />
                </Link>
              </div>

              {/* Forecasting Dashboard */}
              <div className="animate-scaleIn delay-200 opacity-0">
                <div className="bg-slate-900/90 backdrop-blur-xl rounded-[28px] p-10 border border-indigo-500/30 shadow-2xl shadow-indigo-500/10">
                  <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-800/50">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center">
                        <BarChart3 className="w-5 h-5 text-white" />
                      </div>
                      <div>
                        <div className="text-white font-bold">Forecast Dashboard</div>
                        <div className="text-xs text-slate-400">Q1 2024 Demand Forecast</div>
                      </div>
                    </div>
                    <div className="px-3 py-1.5 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                      <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">Active</span>
                    </div>
                  </div>
                  
                  {/* Forecast Metrics */}
                  <div className="space-y-4 mb-6">
                    <div className="bg-gradient-to-r from-indigo-500/10 to-indigo-500/10 rounded-xl p-4 border border-indigo-500/20">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <Activity className="w-4 h-4 text-indigo-400" />
                          <span className="text-xs font-bold text-indigo-400">AI FORECASTING</span>
                        </div>
                        <div className="px-2 py-1 bg-indigo-500/20 rounded-full">
                          <span className="text-[10px] font-bold text-indigo-300">94% CONFIDENCE</span>
                        </div>
                      </div>
                      <p className="text-sm text-white font-medium mb-2">Product Category A - Next 30 Days</p>
                      <div className="flex items-center gap-2 text-xs text-slate-400">
                        <TrendingUp className="w-3 h-3 text-emerald-400" />
                        <span>+12% vs. baseline forecast</span>
                      </div>
                    </div>
                    
                    <div className="bg-slate-800/50 rounded-xl p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <Package className="w-4 h-4 text-indigo-400" />
                          <span className="text-xs font-bold text-indigo-400">INVENTORY OPTIMIZATION</span>
                        </div>
                        <div className="px-2 py-1 bg-emerald-500/20 rounded-full">
                          <span className="text-[10px] font-bold text-emerald-400">OPTIMAL</span>
                        </div>
                      </div>
                      <p className="text-sm text-slate-300 mb-2">Recommended stock level: 2,450 units</p>
                      <div className="text-xs text-slate-400">Reduces carrying cost by 28%</div>
                    </div>

                    <div className="bg-slate-800/50 rounded-xl p-4">
                      <div className="flex items-center gap-3 mb-2">
                        <LineChart className="w-4 h-4 text-indigo-400" />
                        <span className="text-xs font-bold text-slate-400">TREND ANALYSIS</span>
                      </div>
                      <p className="text-sm text-slate-300">Seasonal pattern detected: +15% Q4</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-3 pt-6 border-t border-slate-800/50">
                    <div className="bg-slate-800/30 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <BarChart3 className="w-3 h-3 text-indigo-400" />
                        <span className="text-[10px] font-bold text-slate-500 uppercase">Accuracy</span>
                      </div>
                      <div className="text-xl font-bold text-indigo-400">94%</div>
                    </div>
                    <div className="bg-slate-800/30 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <TrendingUp className="w-3 h-3 text-indigo-400" />
                        <span className="text-[10px] font-bold text-slate-500 uppercase">Forecasts</span>
                      </div>
                      <div className="text-xl font-bold text-indigo-400">1,247</div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* Context Section */}
        <section className="py-24 bg-slate-950">
          <div className="container mx-auto max-w-4xl px-6 text-center">
            <p className="text-xl text-slate-400 leading-relaxed">
              Planning needs intelligence, not just algorithms. Traditional forecasting models miss trends, seasonality, and market signals — leading to excess inventory or costly stockouts. <span className="text-indigo-400 font-semibold">Forecasting needs agentic reasoning that adapts.</span>
            </p>
          </div>
        </section>

        {/* Key Problems */}
        <section className="py-32 bg-[#020617]">
          <div className="container mx-auto max-w-7xl px-6">
            <div className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold text-white mb-4">Key Forecasting Challenges</h2>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {problems.map((item, i) => (
                <div
                  key={i}
                  className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 p-10 rounded-2xl hover:border-indigo-500/30 hover:-translate-y-2 hover:scale-[1.02] transition-all"
                >
                  <div className={`w-12 h-12 rounded-xl ${item.bg} flex items-center justify-center mb-6 border border-indigo-500/20`}>
                    <item.icon className={item.color} size={24} />
                  </div>
                  <h3 className="text-xl font-bold text-white mb-3">{item.title}</h3>
                  <p className="text-slate-400 leading-relaxed">{item.desc}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Solution Overview */}
        <section className="py-32 bg-slate-950">
          <div className="container mx-auto max-w-6xl px-6">
            <div className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold text-white mb-4">
                The Forecasting Engine combines statistical models with agentic reasoning
              </h2>
              <p className="text-lg text-slate-400 max-w-3xl mx-auto">
                It continuously analyzes historical data, identifies patterns and trends, adjusts forecasts based on market signals, and optimizes inventory levels automatically.
              </p>
            </div>
            
            <div className="bg-slate-900/50 backdrop-blur-sm rounded-[2.5rem] border border-slate-800/50 p-12">
              <div className="grid md:grid-cols-2 gap-8">
                <div>
                  <h3 className="text-2xl font-bold text-white mb-6">What it does:</h3>
                  <ul className="space-y-4">
                    {[
            "Analyzes historical data and external factors",
            "Identifies patterns, trends, and seasonality",
            "Adjusts forecasts based on market signals",
            "Optimizes inventory levels automatically",
            "Provides explainable predictions with confidence scores",
                    ].map((item, i) => (
                      <li key={i} className="flex gap-3 items-start">
                        <CheckCircle className="w-5 h-5 text-indigo-400 shrink-0 mt-0.5" />
                        <span className="text-slate-300">{item}</span>
                      </li>
                    ))}
                  </ul>
                </div>
                <div className="bg-indigo-500/5 rounded-2xl p-8 border border-indigo-500/10">
                  <h4 className="text-sm font-bold text-indigo-400 uppercase tracking-wider mb-4">Integration Points</h4>
                  <div className="flex flex-wrap gap-2">
                    {channels.map((channel, i) => (
                      <span
                        key={i}
                        className="px-3 py-1.5 bg-slate-800/50 border border-slate-700/50 rounded-full text-sm text-slate-300"
                      >
                        {channel}
                      </span>
                    ))}
                  </div>
                </div>
              </div>
            </div>
          </div>
        </section>

        {/* How It Works */}
        <section className="py-32 bg-[#020617]">
          <div className="container mx-auto max-w-7xl px-6">
            <div className="text-center mb-16">
              <h2 className="text-4xl lg:text-5xl font-bold text-white mb-4">How It Works</h2>
            </div>
            <div className="grid md:grid-cols-5 gap-6">
              {[
            {
              number: "01",
              icon: Database,
              title: "Data Ingestion",
              description: "Ingests sales, inventory, and market data",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              number: "02",
              icon: Brain,
              title: "Pattern Analysis",
              description: "Identifies trends and seasonality patterns",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              number: "03",
              icon: BarChart3,
              title: "Forecast Generation",
              description: "Generates predictions using AI and statistical models",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              number: "04",
              icon: Package,
              title: "Inventory Optimization",
              description: "Recommends optimal inventory levels",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              number: "05",
                  icon: TrendingUp,
              title: "Continuous Learning",
              description: "Improves accuracy from outcomes",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
                },
              ].map((step, i) => (
                <div
                  key={i}
                  className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 p-8 rounded-2xl text-center"
                >
                  <div className={`w-16 h-16 rounded-xl ${step.bg} flex items-center justify-center mx-auto mb-4 border border-indigo-500/20`}>
                    <step.icon className={step.color} size={28} />
                  </div>
                  <div className="text-4xl font-black text-slate-800 mb-4">{step.number}</div>
                  <h3 className="text-lg font-bold text-white mb-2">{step.title}</h3>
                  <p className="text-sm text-slate-400 leading-relaxed">{step.description}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Capabilities */}
        <section className="py-20 bg-slate-950">
          <div className="container mx-auto max-w-7xl px-6">
            <div className="text-center mb-12">
              <h2 className="text-4xl lg:text-5xl font-bold text-white mb-4">Key Capabilities</h2>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {[
            {
              icon: LineChart,
              title: "Demand Forecasting",
              description: "AI-powered demand prediction with high accuracy",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              icon: Package,
              title: "Inventory Optimization",
              description: "Optimal stock levels to minimize costs",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              icon: TrendingUp,
              title: "Trend & Seasonality Analysis",
              description: "Identifies patterns and seasonal variations",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              icon: BarChart3,
              title: "Confidence Scoring",
              description: "Every forecast includes confidence metrics",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              icon: Database,
              title: "Multi-source Data Integration",
              description: "Combines internal and external data sources",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
            },
            {
              icon: Brain,
              title: "Explainable Predictions",
              description: "Understand why forecasts were made",
                  color: "text-indigo-400",
                  bg: "bg-indigo-500/10",
                },
              ].map((capability, i) => (
                <div
                  key={i}
                  className="bg-slate-900/50 backdrop-blur-sm border border-slate-800/50 p-8 rounded-2xl hover:border-indigo-500/30 transition-all"
                >
                  <div className={`w-12 h-12 rounded-xl ${capability.bg} flex items-center justify-center mb-4 border border-indigo-500/20`}>
                    <capability.icon className={capability.color} size={24} />
                  </div>
                  <h3 className="text-xl font-bold text-white mb-2">{capability.title}</h3>
                  <p className="text-slate-400 leading-relaxed">{capability.description}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* Outcomes */}
        <section className="py-20 bg-[#020617]">
          <div className="container mx-auto max-w-4xl px-6">
            <div className="bg-slate-900 rounded-[2.5rem] border border-slate-800 shadow-2xl overflow-hidden">
              <div className="p-8 md:p-12 border-b border-slate-800 text-center">
                <h2 className="text-3xl font-bold text-white mb-2">Business Outcomes</h2>
                <p className="text-slate-400">Typical impact from Forecasting Engine deployment</p>
              </div>
              <div className="divide-y divide-slate-800">
                {[
                  { metric: "Forecast Accuracy", improvement: "+40%", color: "text-indigo-400" },
                  { metric: "Inventory Levels", improvement: "↓ 30%", color: "text-indigo-400" },
                  { metric: "Stockout Rate", improvement: "↓ 50%", color: "text-indigo-400" },
                  { metric: "Planning Speed", improvement: "80% faster", color: "text-indigo-400" },
                  { metric: "ROI Period", improvement: "3-6 months", color: "text-indigo-400", highlight: true },
                ].map((row, i) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between p-8 hover:bg-slate-800/20 transition-colors ${row.highlight ? "bg-indigo-500/5" : ""}`}
                  >
                    <span className="text-lg font-medium text-slate-300">{row.metric}</span>
                    <div className="text-right">
                      <div className={`text-2xl font-bold ${row.highlight ? "text-indigo-400" : row.color}`}>
                        {row.improvement}
                      </div>
                      <div className="text-xs uppercase font-bold text-slate-500 tracking-tighter">
                        {row.highlight ? "Typical Period" : "Improvement"}
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            </div>
          </div>
        </section>

        {/* CTA Section */}
        <section className="py-20 container mx-auto px-6">
          <div className="bg-gradient-to-br from-indigo-600 to-violet-700 rounded-[3rem] p-12 md:p-20 text-center relative overflow-hidden shadow-2xl">
            <div className="absolute top-0 right-0 w-64 h-64 bg-indigo-500/20 blur-[100px]" />
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 relative z-10">
              Plan with clarity, not guesswork.
            </h2>
            <p className="text-slate-100 text-lg mb-10 max-w-xl mx-auto relative z-10">
              See the Forecasting Engine in action with a personalized demo.
            </p>
            <Link 
              to="/contact"
              className="inline-flex items-center justify-center bg-white text-slate-900 hover:bg-slate-100 rounded-2xl h-14 px-10 font-bold transition-all relative z-10"
            >
              Request Demo
            </Link>
          </div>
        </section>
      </main>
      <Footer />
    </div>
  );
};

export default ForecastingEngine;
